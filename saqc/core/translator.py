...
class Translator:
    def __init__(self, forward: Dict[UserFlag, float], backward: Optional[Dict[float, UserFlag]]=None):
        ...
        if {UNFLAGGED, BAD} - set(backward.keys()):
            raise ValueError(f"need translations for the special flags `UNFLAGGED` ({UNFLAGGED}) and `BAD` ({BAD})")
...
class FloatTranslator(Translator):
    def __init__(self):
        super().__init__({-np.inf: -np.inf, **{k: k for k in np.arange(0, 256, dtype=float)}})


class DmpTranslator(Translator):

    _FORWARD: Dict[str, float] = {"NIL": UNFLAGGED, "OK": GOOD, "DOUBTFUL": DOUBTFUL , "BAD": BAD}
    ...
    def backward(self, flags: Flags, call_stack: CallStack) -> pd.DataFrame:
        ...
        for field in tflags.columns:
            ...
            var_flags = {
                ...
                "quality_comment" : flag_pos.apply(lambda p: json.dumps({"test": flag_funcs[p]})),
                "quality_cause"   : "",
            }
...
class PositionalTranslator(Translator):
    ...
    _BACKWARD: Dict[float, int] = {UNTOUCHED: 0, UNFLAGGED: 0, GOOD: 0, DOUBTFUL: 1, BAD: 2}
