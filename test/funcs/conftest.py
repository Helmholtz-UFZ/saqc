...
def course_1(char_dict):
    ...
    def fix_funk(freq='10min', periods=10, initial_level=0, peak_level=10,
        ...
        data = np.append(np.linspace(initial_level, peak_level, int(np.floor(len(t_index) / 2))),
...
def course_3(char_dict):
    ...
    def fix_funk(freq='10min', periods=10, initial_level=0, final_level=2, out_val=-5,
        ...
        insertion_index = pd.DatetimeIndex([ind1 + crowd_spacing*pd.Timedelta(str(k) + 'min') for k in range(1,crowd_size+1)])
        data.iloc[int(np.floor(periods / 2))] = out_val
