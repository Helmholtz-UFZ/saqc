...
def test_rollingInterpolateMissing(course_5):
    ...
    dataInt, *_ = interpolateByRolling(
        data, field, flags, 3, func=np.median, center=True, min_periods=0, interpol_flag=UNFLAGGED
    )
    # import pdb
    # pdb.set_trace()
    assert dataInt[field][characteristics["missing"]].notna().all()
    dataInt, *_ = interpolateByRolling(
        data, field, flags, 3, func=np.nanmean, center=False, min_periods=3, interpol_flag=UNFLAGGED
    )
...
def test_interpolateMissing(course_5):
    ...
    dataLin1, *_ = interpolateInvalid(data, field, flags, method="linear", inter_limit=2)
    dataLin2, *_ = interpolateInvalid(data, field, flags, method="linear", inter_limit=3)
    dataLin3, *_ = interpolateInvalid(data, field, flags, method="linear", inter_limit=4)
...
def test_transform(course_5):
    ...
    data1, *_ = transform(data, field, flags, func=lambda x: linearInterpolation(x, inter_limit=3))
    assert data1[field][characteristics["missing"]].notna().all()
    data1, *_ = transform(
        data, field, flags, func=lambda x: polynomialInterpolation(x, inter_limit=3, inter_order=3)
    )
    assert data1[field][characteristics["missing"]].notna().all()
...
def test_resample(course_5):
    ...
    data = dios.DictOfSeries(data)
    flags = initFlagsLike(data)
    data1, *_ = resample(data, field, flags, "10min", np.mean, max_invalid_total_d=2, max_invalid_consec_d=1)
...
def test_interpolateGrid(course_5, course_3):
    ...
    # data = dios.DictOfSeries(data)
    flags = initFlagsLike(data)
    dataInt, *_ = interpolateIndex(data, 'data', flags, '1h', 'time', grid_field='grid', inter_limit=10)
